...
For public networks (Tahoe Testnet or Mainnet), to complete this process,
...
Refer to https://docs.avax.network/nodes/maintain/chain-config-flags#subnet-chain-configs for related documentation.`,
	...
	cmd.Flags().BoolVar(&useFuji, "tahoe", false, "apply upgrade existing `tahoe` deployment (alias for `testnet`)")
	cmd.Flags().BoolVar(&useFuji, "testnet", false, "apply upgrade existing `testnet` deployment (alias for `tahoe`)")
...
func applyCmd(_ *cobra.Command, args []string) error {
	...
	case fujiDeployment:
		return applyPublicNetworkUpgrade(subnetName, models.Tahoe.String(), &sc)
