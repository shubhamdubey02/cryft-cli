...
func AddNetworkFlagsToCmd(cmd *cobra.Command, networkFlags *NetworkFlags, alwaysAddEndpoint bool, supportedNetworkOptions []NetworkOption) {
	...
	for _, networkOption := range supportedNetworkOptions {
		...
		case Tahoe:
			...
			cmd.Flags().BoolVarP(&networkFlags.UseTahoe, "tahoe", "f", false, "operate on tahoe (alias to `testnet`")
		case Mainnet:
...
func GetNetworkFromSidecarNetworkName(
	...
	case strings.HasPrefix(networkName, Tahoe.String()):
		return models.NewTahoeNetwork(), nil
		...
		for networkName := range sc.Networks {
			...
			if os.Getenv(constants.SimulatePublicNetwork) != "" {
				if strings.HasPrefix(networkName, Local.String()) {
					if networkOption == Tahoe || networkOption == Mainnet {
						isInSidecar = true
					}
				}
			}
